...
class CausalConv3d(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        time_pad_1 = time_pad // 2
        time_pad_2 = time_pad - time_pad_1
...
class UpsampleBlock(nn.Module):
    ...
    def forward(self, x):
        x = nn.functional.interpolate(
            x, scale_factor=(2, 1, 1), mode="nearest"
        )
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.num_res_blocks):
            self.res_blocks.append(
                ResBlock(filters, filters, **self.block_args)
            )

        self.upsample_block = UpsampleBlock(
            filters, filters, **self.block_args
        )
...
class MUGVAETemporal(nn.Module):
    def __init__(
        self,
        in_out_channels=4,
        latent_embed_dim=4,
        ...
        self.out_channels = latent_embed_dim  # in_out_channels
    ...
    def encode(self, x):
        time_padding = (
            ...
            - x.shape[2] % self.time_downsample_factor
        )
        x = pad_at_dim(x, (time_padding, 0), dim=2)
        B, C, T, H, W = x.shape
        x = rearrange(x, "b c (t ts) h w -> (b t) c ts h w", ts=8)
        encoded_feature = self.encoder(x)
        moments = self.quant_conv(encoded_feature).to(x.dtype)
        moments = rearrange(moments, "(b t) c ts h w -> b c (t ts) h w", b=B)
    ...
    def decode(self, z, num_frames=None):
        time_padding = (
            ...
            - num_frames % self.time_downsample_factor
        )
