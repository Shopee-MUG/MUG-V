    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                if image is not None and mask is not None:
                    sample = sample * mask + im_x[str(tuple(sample.shape))] * (
                        1 - mask
                    )
            else:
                # middle
    ...
    def __init__(
        ...
        for i, num_block in enumerate(num_blocks):
            ...
            if i == 0:
                layers.append(
                    nn.Conv2d(
                        in_channels, num_channels, kernel_size=3, padding=1
                    )
                )
            else:
                layers.append(
            ...
            for _ in range(num_block):
                layers.append(
                    AutoencoderTinyBlock(num_channels, num_channels, act_fn)
                )
    ...
    def __init__(
        ...
        for i, num_block in enumerate(num_blocks):
            ...
            if not is_final_block:
                layers.append(
                    nn.Upsample(
                        scale_factor=upsampling_scaling_factor,
                        mode=upsample_fn,
                    )
                )
